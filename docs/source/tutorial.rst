...
Fields that can be modified: `title`, `description`, `status`, `period.endDate`, `items`, `terminationDetails`, `milestones` in follow statuses **pending** and **scheduled**.
...
Before contract can be completed all ``milestones`` fields ``amountPaid.amount`` and ``status`` should be set (regardless whether the contract was successful or unsuccessful).
Contract can be completed by switching to ``terminated`` status.

Let's view all contract milestones

.. include:: tutorial/contract-all-milestones.http
   :code:
...
Note that you can set/change only ``status`` value.
